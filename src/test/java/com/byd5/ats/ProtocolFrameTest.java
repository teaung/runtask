...
/*	public class BeanWithCreator {
	...
	public void TestFrameCIStatus() {
		...
		Short signal0 = 193;
		Byte switch0 = 113;
		assertEquals(signal0, frameCIStatus.getCiStatus().getSignalStatus().get(0));
		assertEquals(switch0, frameCIStatus.getCiStatus().getSwitchStatus().get(0));
		
		int interfaceType = AppProtocolConstant.IFACETYPE_CI2ATS;//0x9009;
		assertEquals(interfaceType, frameCIStatus.getFrameHeader().getInterfaceType());
		short msgType = AppProtocolConstant.MSGTYPE_CI_CI_STATUS; //0x0202;
		assertEquals(msgType, frameCIStatus.getMsgHeader().getType());
	...
	public void TestFrameZCTSRStatus() {
		...
		int interfaceType = AppProtocolConstant.IFACETYPE_ZC2ATS; //0x9004;
		assertEquals(interfaceType, frame.getFrameHeader().getInterfaceType());
		short msgType = AppProtocolConstant.MSGTYPE_ZC_TSR_STATUS; //0x0206;
		assertEquals(msgType, frame.getMsgHeader().getType());

		short logicNum = 92;
		Short tsrValue0 = 4;
		
		assertEquals(logicNum, frame.getTsrStatus().getLogicNum());
	...
	public void TestFrameTrainStatus() {
		...
		int interfaceType = AppProtocolConstant.IFACETYPE_ATS2VOBC; //0x9005;
		assertEquals(interfaceType, frame.getFrameHeader().getInterfaceType());
		short msgType = AppProtocolConstant.MSGTYPE_VOBC_TRAIN_STATUS; //0x0204;
		assertEquals(msgType, frame.getMsgHeader().getType());
		...
		assertEquals(trainCode, frame.getTrainStatus().getTrainCode());
		assertEquals(sec, frame.getTimestamp().getSec());
		assertEquals(usec, frame.getTimestamp().getUsec());
	...
	public void TestFrameATOStatus() {
		...
		int interfaceType = AppProtocolConstant.IFACETYPE_ATS2VOBC; //0x9005;
		assertEquals(interfaceType, frame.getFrameHeader().getInterfaceType());
		short msgType = AppProtocolConstant.MSGTYPE_VOBC_ATO_STATUS; //0x0202;
		assertEquals(msgType, frame.getMsgHeader().getType());
		...
		assertEquals(trainNum, frame.getAtoStatus().getTrainNum());
		assertEquals(sec, frame.getTimestamp().getSec());
		assertEquals(usec, frame.getTimestamp().getUsec());
